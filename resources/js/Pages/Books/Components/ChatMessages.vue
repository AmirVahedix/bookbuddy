    ...
    <div>
        <div v-if="props.messages && props.messages.length" class="border-t border-slate-100 dark:border-slate-800/80 pt-8 mt-12 space-y-8">
            <div class="flex items-center gap-2 mb-4">
                ...
                <span class="text-xs font-black uppercase tracking-wider text-violet-600 dark:text-violet-400">Discussion & Questions</span>
            </div>
            ...
            >
                ...
                <div
                    v-if="msg.role === 'user'"
                    class="max-w-[85%] rounded-2xl px-4 py-2.5 bg-violet-600 text-white rounded-tr-none text-xs font-medium shadow-sm shadow-violet-500/10"
                >
                    {{ msg.content }}
                </div>
                <!-- Assistant response rendered exactly like the summary content -->
                <div
                    v-else
        ...
        <div v-if="props.processing" class="flex flex-col items-start mt-6 border-t border-slate-100 dark:border-slate-800/80 pt-6">
            <div class="text-[9px] text-slate-400 dark:text-slate-500 mb-1 px-1 font-semibold">AI Assistant</div>

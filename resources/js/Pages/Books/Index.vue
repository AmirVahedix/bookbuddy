...
const filters = computed(() => [
    { label: t('status_all'), value: null },
    { label: t('status_reading'), value: 'currently_reading', colorClass: 'border-violet-500/20 text-violet-600 dark:text-violet-400 bg-violet-500/5' },
    { label: t('status_completed'), value: 'done', colorClass: 'border-emerald-500/20 text-emerald-600 dark:text-emerald-400 bg-emerald-500/5' },
    { label: t('status_unread'), value: 'planned_for_future', colorClass: 'border-slate-500/20 text-slate-600 dark:text-slate-400 bg-slate-500/5' },
]);
        ...
        <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-[calc(2rem+env(safe-area-inset-top))] sm:pt-8 pb-24 sm:pb-8">
            ...
            <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="text-start">
                    <h1 class="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">{{ t('library') }}</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-2">{{ t('library_subtitle') }}</p>
                </div>
            ...
            <div v-if="tags.length > 0" class="flex flex-wrap items-center gap-2 mb-8 -mt-4 pb-1">
                <span class="text-xs font-bold text-slate-400 dark:text-slate-500 me-2 uppercase tracking-wider">Tags:</span>
                <Link
                        ...
                        >
                            ...
                            <div class="mt-3 pointer-events-auto">
                                <div class="flex items-center justify-between text-[11px] font-semibold text-white/90 mb-1">
                                    <span class="text-white/70">{{ t('page') }} {{ book.current_page }} {{ t('of') }} {{ book.total_pages }}</span>
                                    <span
            ...
            <div v-else class="rounded-3xl border border-slate-200 dark:border-slate-900 bg-white/60 dark:bg-slate-900/20 p-8 text-center transition-colors duration-200 shadow-sm">
                <div class="max-w-md mx-auto py-12">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800 dark:text-white">{{ t('no_books_found') }}</h3>
            ...
            <div class="relative w-full max-w-md transform overflow-hidden rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 text-start shadow-2xl transition-all animate-in fade-in zoom-in-95 duration-200">
                ...
                <div class="mt-6 flex items-center justify-end gap-3">
                    ...
                    >
                        ...
                        {{ isDeleting ? t('updating') : t('delete_book') }}
                    </button>
                </div>
            </div>
        </div>
